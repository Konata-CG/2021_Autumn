...
def AStarSearch(Graph, start, end, distances):
    ...
    explored = []
    queue = []
    frontier.append((start, distances[start], 0))
    while len(frontier) != 0:
        ...
        for neighbor in Graph[state[0]]:
            if neighbor[0] not in explored:
                distance_from_start = state[2] + neighbor[1]
                total_distance = distance_from_start + distances[neighbor[0]]
                frontier.append((neighbor[0], total_distance, distance_from_start))
